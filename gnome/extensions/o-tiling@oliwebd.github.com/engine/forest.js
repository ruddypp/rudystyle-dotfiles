...
export class Forest extends Ecs.World {
    ...
    arrange(ext, workspace, _ignore_reset = false) {
    ...
    *iter(entity, kind = null) {
    ...
    largest_window_on(ext, entity) {
        ...
        let largest_size = 0;
        const window_compare = (entity, require_tilable) => {
            const window = ext.windows.get(entity);
        ...
        if (largest_window === null) {
            ...
            if (largest_window !== null) {
                log.warn(`largest_window_on: fork ${entity} had no tilable window; ` +
...
function move_window(ext, window, rect, on_complete, retries = 0) {
