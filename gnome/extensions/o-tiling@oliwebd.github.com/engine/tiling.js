...
export class Tiler {
    ...
    swap_window = null;
    queue = new exec.ChannelExecutor();
    constructor(ext) {
        this.keybindings = {
        ...
        if (this.window) {
            ...
            if (meta) {
                ...
                if (!tree_swapped) {
                    ...
                    meta.move(ext, Rect.Rectangle.from_meta(ext.overlay), () => {
...
function tile_monitors(rect) {
    const total_size = (a, b) => a.width * a.height - b.width * b.height;
    const workspace = global.workspace_manager.get_active_workspace();
    return Main.layoutManager.monitors
        .map((_, i) => Rect.Rectangle.from_meta(workspace.get_work_area_for_monitor(i)))
        .filter((monitor) => {
